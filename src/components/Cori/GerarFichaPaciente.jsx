...
const GerarFichaAtor = () => {
  ...
  const handleSubmit = () => {
    ...
    const requestBody = {
      patient: sections.dadosPaciente.map(({ title, content }) => ({
        title,
        content,
      })),
      vitals: sections.dadosVitais.map(({ title, content }) => ({
        title,
        content,
      })),
      exams: sections.examesFisicos.map(({ title, content }) => ({
        title,
        content,
      })),
    };
    ...
    fetch(
      `https://back-end-cori-feedbacks-node.opatj4.easypanel.host/cori/patient-record/${caseId}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  return (
    <div className="gerar-ficha-container">
            ...
            {displayedCases.map((caso) => {
              ...
              return (
                <tr key={caso.uuid}>
                  ...
                  <td>
                    {caso.patients && caso.patients.length > 0 ? (
                      <span>✓</span>
                    ) : (
                      <span>X</span>
                    )}
      ...
      {currentStep === 1 && (
        <Section
          title="Dados Vitais"
          items={sections.dadosVitais}
          onAdd={() => handleAddItem("dadosVitais")}
          onInputChange={(i, field, val) =>
            handleInputChange("dadosVitais", i, field, val)
          }
          onRemove={(i) => handleRemoveItem("dadosVitais", i)}
        />
      )}

      {currentStep === 2 && (
        <Section
          title="Exames Físico Pré-Realizados"
